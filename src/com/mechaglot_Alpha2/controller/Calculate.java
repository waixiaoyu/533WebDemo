...
public class Calculate {
	...
	public float calculateNNSimilarity(String a, String b) {
		...
		String predict = block + "," + ChapmanLengthDeviation + "," + CMSoundex + "," + ChapmanMeanLength + ","
				...
				+ "," + SmithWatermanGotoh + "," + SmithWatermanGotohWindowedAffine + "," + TagLinkToken + "," + Soundex
				+ "," + vectorSpace;
	...
	private float getRBFSemantics(String in) {
		...
		try {
			if ((rss == null)) {
				this.rss = (RBFRegressor) weka.core.SerializationHelper.read(prefix+"mechaglot_model/Semantic_RBFR.model");
			}
			Instance first = instanceMaker(in);
